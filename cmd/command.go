...
func (cmd Command) Execute(args string, ctx Context) (res safetypes.Result[discord.MessageCreate]) {
	...
	for _, v := range cmd.fields {
		...
		cp.Elem().Set(v)
		msg, line, err := cmd.executeRunnable(cp, args, ctx)
		if err == nil {
	...
	// stop there.
	return res.Err(leastErroneous.Error())
}
...
func (cmd Command) Params(ctx Context) [][]ParamInfo {
	params := make([][]ParamInfo, 0, len(cmd.fields))
	for _, runnable := range cmd.fields {
		elem := reflect.New(runnable.Type()).Elem()
		elem.Set(runnable)

		if allower, ok := runnable.Interface().(Allower); ok && !allower.Allow(ctx) {
...
func (cmd Command) Runnables(ctx Context) map[int]Runnable {
	m := make(map[int]Runnable, len(cmd.fields))
	for i, runnable := range cmd.fields {
...
func (cmd Command) executeRunnable(v reflect.Value, args string, ctx Context) (*discord.MessageCreate, *Line, error) {
	if a, ok := v.Interface().(Allower); ok && !a.Allow(ctx) {
		...
		return nil, nil, fmt.Errorf("You cannot execute this command.")
	}
	...
	if args != "" {
		...
		record, err := r.Read()
		if err != nil {
			return nil, nil, fmt.Errorf("error parsing command string: %w", err)
		}
		argFrags = record
	}
	parser := parser{}
	arguments := &Line{args: argFrags, ctx: ctx}
	...
	if arguments.Len() != 0 {
		return nil, arguments, fmt.Errorf("unexpected '%v'", strings.Join(arguments.args, " "))
	}

	msg := v.Interface().(Runnable).Run(ctx)
	return &msg, arguments, nil
}
